...
data = {
    ...
    "How do you validate a phone number using regex?": r"^\+?[1-9]\d{1,14}$",
    "How do you remove extra spaces from a string using regex?": r"re.sub(r'\s+', ' ', text)",
...
# roslaunch my_robot world.launch
    ...
    "How to extract words from a string?": r"re.findall(r'\b\w+\b', string)",
...
def fibonacci(n):
    ...
    "How to use regular expressions in Python?": r"""
import re
...
# Function for MCQ mode
def mcq_mode(question, options):
    questions = list(data.keys())
    ...
    if matches:
        ...
        if score > 80:
            ...
            best_option = process.extractOne(correct_answer, options)[0]
            return f"Correct Answer: {best_option}"
    return "Could not determine the correct answer."

# Main function to handle user input
def main():
    mode = input("Choose mode - General (G) or MCQ (M): ").strip().upper()
    ...
    if mode == 'G':
        ...
        while True:
            question = input("Enter your MCQ question (or type 'exit' to quit): ")
            if question.lower() == 'exit':
                print("Goodbye!")
                break
            options = [input(f"Option {chr(97 + i)}: ") for i in range(4)]
            print(mcq_mode(question, options))
    else:
